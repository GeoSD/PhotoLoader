...
//  PhotoLoader
//
//  Created by Ивлев Александр on 27/04/2019.
...
final class ViewController: UITableViewController {
    ...
    private var task: URLSessionDownloadTask = URLSessionDownloadTask()
    private let cache: NSCache<AnyObject, UIImage> = NSCache()
    var tempData: Data?
    ...
    override func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        cell.startLoad()
        let stringURL = ViewController.addresses[indexPath.row]
        let url = urls[indexPath.row]
        if cache.object(forKey: stringURL as AnyObject) != nil {
            let cachedImage = cache.object(forKey: stringURL as AnyObject)
